...
contract MeetingScheduler is IMeetingScheduler {
    ...
    function cancelMeeting(uint256 meetingId) external override {
        ...
        require(
            scheduledMeeting.status == MeetingStatus.STARTED, // @note it should be PENDING instead of STARTED
            "can't end a meeting if not started"
        );
        meetings[meetingId].status = MeetingStatus.ENDED; // @note it should be CANCELLED instead of ENDED
    }

    function endMeeting(uint256 meetingId) external override {
        ScheduledMeeting memory scheduledMeeting = meetings[meetingId];
        require(
            scheduledMeeting.status == MeetingStatus.PENDING, // @note it should be STARTED instead of PENDING
        ...
        meetings[meetingId].status = MeetingStatus.CANCELLED; // @note it should be ENDED instead of CANCELLED
    }
