...
rule checkStartedToStateTransition(method f, uint256 meetingId) {
	...
	assert ((stateBefore == 2 && f.selector == joinMeeting(uint256).selector) => getStateById(e, meetingId) == 2), "the status of the meeting changed from STARTED through the joinMeeting function";
}
