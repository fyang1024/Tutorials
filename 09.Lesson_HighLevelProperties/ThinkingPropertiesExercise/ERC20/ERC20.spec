...
invariant totalSupplyNotLessThanSingleUserBalance(address user)
    totalSupply() >= balanceOf(user)
    {
        preserved transfer(address recipient, uint256 amount)  with (env e)  {
            require totalSupply() >= balanceOf(recipient) + balanceOf(e.msg.sender);
        }
        preserved transferFrom(address from, address to, uint256 amount) with (env e) {
            require totalSupply() >= balanceOf(from) + balanceOf(to);
        }
    }
